...
**Large screens:** `monkey.jungle` maps 360/390/416 screens to `resources-large-100` (larger icons only; the dog stays 120px, since 1.5× scaling left uneven pixel-art edges) and 454 to `resources-large-200` (2× sprites with a per-breed `<palette>`, since compile-time scaling otherwise smooths the pixel art into extra colors, plus larger icons). Frame and icon sizes are now read from the loaded bitmaps instead of constants. Packages: ~353KB at 2×; ~225KB elsewhere.
