...
1. **Feet tappies.** Triggers: added to the existing random trick pool (alongside lick/tail-spin/sploot-rear).
2. **Laying down (sploot).** Split, since the two views ended up wanting different treatment: rear view is **done** — moved into the random trick pool above, replacing sit-stand. Front view is **done** too, as the first conditional trigger (a new mechanism, not part of the random pool): fires on low battery, two tiers (`LOW_BATTERY_THRESHOLD_PERCENT` 20%, `CRITICAL_BATTERY_THRESHOLD_PERCENT` 10%, both tentative), same animation replayed as a second warning at the lower tier. Checked once per idle tick via a pure `isLowBattery()` comparison (reused for both thresholds), only from idle, each tier fires once per `onShow()` independently (`mLowBatteryTrickShown`/`mCriticalBatteryTrickShown` — neither refires every tick while battery stays low; crossing straight past 20% to ≤10% in one tick consumes both flags so it plays once, not twice). Reached via a new `startTrick(index)` (generalized from `startRandomTrick()`) and `RANDOM_TRICK_POOL_SIZE`/`SPLOOT_FRONT_TRICK_INDEX`, so conditional tricks reuse the same `mTricks` clip-playback machinery without a parallel state machine. Activity-goal-reached and near-bedtime remain candidates for additional conditional triggers, not yet wired.
