...
2. **Laying down (sploot).** Split, since the two views ended up wanting different treatment: rear view is **done** — moved into the random trick pool above, replacing sit-stand. Front view (`corgi-sploot-front-sheet.png`, 8 frames) is still **pending** as a standalone conditional trigger (not part of the random pool — new mechanism, doesn't exist yet): candidates are low battery, activity goal reached (`Info.steps` vs `.stepGoal`), near-bedtime (`UserProfile.sleepTime`) — which one(s) to wire first still needs deciding.
