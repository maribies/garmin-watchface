...
| Sploot (front view) | Battery ≤ `LOW_BATTERY_THRESHOLD_PERCENT` (20%) or `CRITICAL_BATTERY_THRESHOLD_PERCENT` (10%), checked once per idle tick | 8 frames @ 150ms ≈ 1.2s | Only fires from idle, not mid-trick. Not part of the random pool — reached by key (`:splootFront`) in `mTricks`. Same animation plays for both tiers: once at 20%, again as a second warning at 10%. Suppressed while charging (`SystemStats.charging`, via `shouldAlertLowBattery`), which also re-arms each tier; a tier re-arms too once battery is back above its threshold. Crossing straight to ≤10% plays it once, not twice (triggers sharing a trick are disarmed together). |
| Move-alert | `moveBarLevel` ≥ `MOVE_BAR_LEVEL_MAX` (5), checked once per idle tick | 9 frames @ 130ms ≈ 1.17s | Reuses tail spin's bitmap/clip. Re-arms once the move bar drops below max. |
| High stress (lick) | Stress level ≥ `HIGH_STRESS_THRESHOLD` (76 — Garmin's own "High" band), checked once per idle tick | 11 frames @ 150ms ≈ 1.65s | Reuses the `:lick` trick directly (same key as the random pool). No data (device/firmware doesn't expose `SensorHistory.getStressHistory`) never fires, doesn't error. Re-arms once stress drops below the threshold (or data goes missing). |

**Conditional trigger rule** (`stepTriggers`): each fires once when its condition becomes true and re-arms when it becomes false, independent of `onShow()`. Priority order is `TRIGGER_ORDER`; a lower-priority trigger that's active at the same time plays on a later idle tick instead of being dropped. Move bar and stress come from the once-a-minute cache, so they can lag their condition by up to a minute.
